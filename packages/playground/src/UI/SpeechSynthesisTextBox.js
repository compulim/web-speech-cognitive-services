...
const SpeechSynthesisTextBox = () => {
  ...
  const dispatchSetSpeechSynthesisVoice = useCallback(patterns => {
  ...
  const handleCantoneseClick = useCallback(() => {
    dispatchSetSpeechSynthesisText('一天，悶悶不樂的愛麗絲跟姊姊同坐於河畔。忽見一隻古怪的白兔走過──牠穿戴打扮，手持懷錶，自言自語，行色匆匆。好奇的愛麗絲跟着牠跑，跳進兔子洞裏去。');
  ...
  const handleEnglishClick = useCallback(() => {
    dispatchSetSpeechSynthesisText('Pooh is very social. After Christopher Robin, his closest friend is Piglet, and he most often chooses to spend his time with one or both of them.');
    ...
    <div className="input-group">
      <div className="input-group-prepend">
        ...
        <div className="dropdown-menu">
          ...
          <button
            className="dropdown-item"
            onClick={ handleEnglishSSMLClick }
            type="button"
          >Winnie the Pooh (English in SSML)</button>
          <button
            className="dropdown-item"
            onClick={ handleCantoneseClick }
            type="button"
